...
export const HeaderToggle = React.memo(() => {
	const handleDarkMode = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
		...
		Helpers.AddDarkClass(true);
	};

	const handleWhiteMode = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
		event.preventDefault();
		Helpers.RemoveDarkClass(true);
	};
	...
	return (
		<div className="text-right">
			<span className="px-1 py-[3px] inline-flex border bg-gray-200 rounded-md">
				...
				<button onClick={handleWhiteMode} type="button" className="px-4 py-[3px] z-0 rounded">
					<SunIcon />
				</button>
				<button onClick={handleDarkMode} type="button" className="px-4 py-[3px] z-0 rounded">
					<MoonIcon />
				</button>
			</span>
		</div>
